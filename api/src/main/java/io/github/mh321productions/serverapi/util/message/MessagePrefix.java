...
public final class MessagePrefix {
	...
	public enum PrefixFormat {
		...
		/**
		 * Ein Nebenpräfix, meist kombiniert mit anderen Präfixen: §8[&lt;Präfix&gt;§8]§r
		 */
		Secondary,
	 ...
	 * @param format Ob das Chat-Format hinzugefügt werden soll
	 */
	public MessagePrefix(String prefix, PrefixFormat format) {
        switch (format) {
            case Main -> simplePrefix = prefix + mainFormat;
            case Secondary -> simplePrefix = String.format(secondaryFormat, prefix);
            default -> simplePrefix = prefix;
        }
